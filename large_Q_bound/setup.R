...
run.once<-function(n,sim.id){
    ...
    tmle_c.fold<-lapply(folds,function(fold){
        ...
        list(Qhat=Qhat,epsilon=epsilon)
    })
    ...
    tmle_c_trans.fold<-lapply(folds,function(fold){
        ...
        list(Qhat=Qhat,epsilon=epsilon)
        # 
        # model<-glm(data$Y.trans[fold]~H[fold]+0,family=binomial(),offset=logitQ.trans[fold],subset=data$A[fold]==0)
        # epsilon<-coef(model)
        # Qhat<-expit(logitQ.trans[fold]+epsilon*H[fold])
        # 
        # list(Qhat=Qhat,epsilon=epsilon)
    })
